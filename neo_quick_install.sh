#!/usr/bin/env bash
# ==============================================================================
# Stupid_bot 终极自动化部署脚本 v2.0
# (Python校验 + MemPalace可选 + .env自动生成 + 滚屏进度条 + Docker自选
#  + 动态裁剪 + 皮肤站 + 自动清理)
# ==============================================================================
set -e
...
echo -e "\033[1;36m      Stupid_bot 自动安装与环境部署脚本 v2.0     \033[0m"
echo -e "\033[1;36m=================================================\033[0m"
...
get_password() {
    local title=$1; local msg=$2; local RESULT
    if $USE_TUI; then
        RESULT=$(whiptail --title "$title" --passwordbox "$msg" 10 60 3>&1 1>&2 2>&3)
    else
        read -s -p "[$title] $msg: " RESULT; echo ""
    fi
    echo "$RESULT"
}
...
# 🔒 Python ≥ 3.10 版本校验（pyproject.toml 要求 >=3.10, <4.0）
echo -e "\033[1;30m│\033[0m 正在校验 Python 版本..."
if ! python3 -c "import sys; exit(0 if sys.version_info >= (3, 10) else 1)" 2>/dev/null; then
    ...
    echo "║                                                  ║"
    echo "║  当前版本: $PY_VER"
    echo "║  要求版本: Python 3.10+                          ║"
    echo "║                                                  ║"
    echo "║  请先升级 Python 后再运行本脚本：                ║"
...
echo -e "\033[1;30m│\033[0m ✅ Python 版本校验通过: $PY_VER"
...
if ask_yes_no "NapCat 安装" "是否需要在此服务器上安装 NapCat (QQ 协议端)？"; then
    ...
    # 直接执行脚本，不使用 run_logged 包装，防止管道吞噬返回码或引发异常
    # 并且将报错信息重定向输出，不阻塞脚本
    sudo bash napcat.sh $DOCKER_FLAG --qq "$BOT_QQ" --mode ws --proxy 0 --confirm
    ...
    if [ $NAPCAT_STATUS -ne 0 ]; then
        echo -e "\033[1;33m│ ⚠️ NapCat 部署脚本返回了非 0 状态码 ($NAPCAT_STATUS)，可能遇到了需要交互的选项或网络问题。\033[0m"
        echo -e "\033[1;33m│ ⚠️ 请在安装脚本跑完后，手动检查 NapCat 容器状态。\033[0m"
    else
        echo -e "\033[1;32m│ ✅ NapCat 自动化部署脚本执行完毕！\033[0m"
    fi
else
...
if python3 /tmp/parse_plugins.py; then
    ...
    # 检测是否选择了 server_tools（皮肤站相关）
    if [[ "$SELECTED_PLUGINS" == *"server_tools"* ]] || [[ "$SELECTED_PLUGINS" == *"all"* ]] || [ -z "$SELECTED_PLUGINS" ] && [ -n "${cli_opts[*]}" ]; then
        NEEDS_BS_PROMPT=true
    fi
    
    # 检测是否选择了 prts_ai（AI 中枢插件）
    if [[ "$SELECTED_PLUGINS" == *"prts_ai"* ]] || [ -z "$SELECTED_PLUGINS" ] && [ -n "${cli_opts[*]}" ]; then
        HAS_AI_PLUGIN=true
    fi
fi
...
# 🧩 MemPalace 可选安装：仅在选择了 AI 插件时询问
MEMPALACE_INSTALLED=false
if [ "$HAS_AI_PLUGIN" = true ]; then
    if ask_yes_no "MemPalace 长期记忆" "检测到您选择了 AI 中枢插件 (prts_ai)。\n\n是否安装 MemPalace 以增强 AI 长期记忆功能？\n\n[Yes] : 安装 MemPalace (AI 将获得跨会话记忆检索能力)\n[No]  : 跳过 (不影响基本对话，可稍后手动安装)"; then
        echo -e "\033[1;30m│\033[0m 正在安装 MemPalace 长期记忆组件..."
        run_logged sudo ./venv/bin/pip install mempalace
        MEMPALACE_INSTALLED=true
        echo -e "\033[1;30m│\033[0m ✅ MemPalace 安装成功！"
    else
        echo -e "\033[1;30m│\033[0m 已跳过 MemPalace。可稍后通过以下命令手动安装："
        echo -e "\033[1;30m│\033[0m   source venv/bin/activate && pip install mempalace"
    fi
fi
...
# --- 10. 配置 AI 与环境变量 + 自动生成 .env ---
advance_step "配置 AI 与环境变量"

# 收集超级管理员 QQ（与机器人 QQ 不同）
SUPER_QQ=$(get_input "管理员配置" "请输入超级管理员 QQ 号码 (拥有最高 Bot 权限):\n(可与机器人 QQ 不同)" "$BOT_QQ")

# 收集 Bot 昵称
BOT_NICKNAME=$(get_input "昵称配置" "请输入 Bot 的昵称 (用于 @触发，多个用逗号分隔):" "PRTS,bot")

# 收集 LLM 配置
LLAMA_URL=$(get_input "LLM 配置" "请输入 LLM 推理服务端点 (Llama / vLLM / Ollama 等):" "http://127.0.0.1:11434/v1/chat/completions")

ROUTER_URL=""
if ask_yes_no "LLM 路由配置" "是否配置独立的模型路由 (Router URL)？\n\n如果您有单独的路由/调度服务用于自动判断\nChat/Tech 模式，请选 Yes。\n没有则直接选 No 跳过。"; then
    ROUTER_URL=$(get_input "LLM 路由" "请输入 Router 端点 URL:" "http://127.0.0.1:11434/v1/chat/completions")
fi

# 收集 UID 盐值
UID_SALT=$(get_input "安全配置" "请输入 UID 哈希盐值 (用于用户ID脱敏):" "Stupid_bot!@#")

# MeloTTS 语音合成（可选）
MELOTTS_URL=""
MELOTTS_CONFIGURED=false
if ask_yes_no "语音合成配置" "是否配置 MeloTTS 语音合成服务？\n\n启用后 Bot 可将 AI 回复转为 QQ 语音消息。\n(需要您已单独部署 MeloTTS 服务)"; then
    MELOTTS_URL=$(get_input "MeloTTS 配置" "请输入 MeloTTS FastAPI 服务地址:" "http://127.0.0.1:8082")
    MELOTTS_CONFIGURED=true
fi

# MemPalace 存储路径配置
MEMPALACE_PATH=""
if [ "$MEMPALACE_INSTALLED" = true ]; then
    MEMPALACE_PATH=$(get_input "MemPalace 配置" "请输入 MemPalace 长期记忆数据存储路径:" "$USER_HOME/Stupid_bot/data/mempalace_db")
fi

# Bot 服务端口
BOT_PORT=$(get_input "服务端口" "请输入 Bot 监听端口 (WebUI 也使用此端口):" "8080")

# 🔧 将昵称转换为 JSON 数组格式 ["PRTS", "bot"]
NICKNAME_JSON="["
IFS=',' read -ra NICK_ARR <<< "$BOT_NICKNAME"
for i in "${!NICK_ARR[@]}"; do
    nick=$(echo "${NICK_ARR[$i]}" | xargs)  # 去首尾空格
    if [ $i -gt 0 ]; then NICKNAME_JSON+=", "; fi
    NICKNAME_JSON+="\"$nick\""
done
NICKNAME_JSON+="]"

# 自动生成 .env 文件
ENV_FILE="$USER_HOME/Stupid_bot/.env"
echo -e "\033[1;30m│\033[0m 正在生成 .env 配置文件..."

cat > "$ENV_FILE" << ENVEOF
# ╔══════════════════════════════════════════════════════╗
# ║      Stupid_bot 环境配置文件 (由安装脚本自动生成)     ║
# ╚══════════════════════════════════════════════════════╝
...
if [ "$MELOTTS_CONFIGURED" = true ] || [ "$MEMPALACE_INSTALLED" = true ]; then
    cat >> "$ENV_FILE" << ENVEOF
...
if [ "$MELOTTS_CONFIGURED" = true ]; then
    cat >> "$ENV_FILE" << ENVEOF
# MeloTTS 语音合成 FastAPI 地址
MELOTTS_API_URL="${MELOTTS_URL}"
ENVEOF
fi
...
echo -e "\033[1;30m│\033[0m ✅ .env 配置文件已自动生成: $ENV_FILE"
...
if ask_yes_no "数据迁移" "您是否有旧的数据库备份文件 (stupid_db.sql) 需要迁移？\n\n选 [Yes]: 暂停等待您上传备份文件。\n选 [No] : 自动导入自带的基础结构。"; then
    ...
    if [ -f "$DEFAULT_SQL" ]; then
        ...
        echo -e "\033[1;30m│\033[0m ✅ 基础数据表结构已成功导入！(ai_messages + users + 自动清理事件)"
    else
        echo -e "\033[1;33m│ ⚠️ 未找到 $DEFAULT_SQL，请稍后手动导入数据库结构。\033[0m"
    fi
fi

# --- 12. Blessing Skin 自动化部署 ---
BS_MSG=""
...
# --- 14. 终极提示与唤起 NapCat 扫码 ---

# 构建功能状态摘要
FEATURE_MSG=""
if [ "$MEMPALACE_INSTALLED" = true ]; then
    FEATURE_MSG="${FEATURE_MSG}\n- ✅ AI 长期记忆 (MemPalace): 已启用"
else
    if [ "$HAS_AI_PLUGIN" = true ]; then
        FEATURE_MSG="${FEATURE_MSG}\n- ⬜ AI 长期记忆 (MemPalace): 未安装 (可稍后 pip install mempalace)"
    fi
fi
if [ "$MELOTTS_CONFIGURED" = true ]; then
    FEATURE_MSG="${FEATURE_MSG}\n- ✅ 语音合成 (MeloTTS): ${MELOTTS_URL}"
fi
...
- 超级管理员: [ ${SUPER_QQ} ]
- 数据库账户: [ ${DB_USER} ]
- WebUI 控制台: [ http://服务器IP:${BOT_PORT}/ui/ ]
${BS_MSG}
${FEATURE_MSG}

【✅ .env 已自动生成】：
配置文件已写入 $ENV_FILE
启动后也可通过 WebUI 控制台在线修改任何配置项。
