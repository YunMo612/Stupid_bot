// ==========================================
// PRTS 终端控制台 - 核心逻辑脚本 (稳定版 v14)
// ==========================================

let uiSettings = { anim: true, glass: true, md3: false };
...
window.resetBackground = function() {
    ...
    if(msg) { 
        msg.style.color = "var(--log-info)"; 
        msg.innerText = "✔️ 已恢复默认"; 
        ...
        reader.onload = (evt) => {
            ...
            img.onload = () => {
                ...
                try { 
                    ...
                    msg.style.color = "var(--log-success)"; 
                    msg.innerText = "✔️ 壁纸已保存！"; 
                } catch(err) { 
                    msg.style.color = "var(--log-error)"; 
                    msg.innerText = "❌ 图片过大"; 
                }
...
function performThemeSwitch() {
    ...
    const themeIcon = document.querySelector('#theme-btn .material-symbols-outlined');
...
window.toggleTheme = async function(event) {
    ...
    if (!document.startViewTransition || !uiSettings.anim) { 
        performThemeSwitch(); return; 
    } 
...
window.checkSystem = async function() { 
    const box = document.getElementById('status-box'); box.innerText = '正在探测...'; 
    try { const res = await fetch('/api/system/status'); box.innerHTML = `<span class="log-info">[SYSTEM]</span>\n${JSON.stringify(await res.json(), null, 2)}`; } 
    catch (e) { box.innerHTML = `<span class="log-error">[ERROR]</span> ${e.message}`; } 
}

window.checkAI = async function() { 
    const box = document.getElementById('status-box'); box.innerText = 'PING 大模型...'; 
    try { const res = await fetch('/api/ai/status', {method: 'POST'}); box.innerHTML = `<span class="log-success">[AI CORE]</span>\n${JSON.stringify(await res.json(), null, 2)}`; } 
    catch (e) { box.innerHTML = `<span class="log-error">[ERROR]</span> 未响应`; } 
}
...
function startDeleteCountdown(btnId, originalText) {
    ...
    if (!btn) return;
    let count = 10; btn.disabled = true; btn.innerText = `${originalText} (${count}s)`;
    clearInterval(deleteTimer); 
    deleteTimer = setInterval(() => {
        count--;
        if (count > 0) { btn.innerText = `${originalText} (${count}s)`; } 
        else { clearInterval(deleteTimer); btn.disabled = false; btn.innerText = originalText; }
    }, 1000);
}
function stopDeleteCountdown(btnId, originalText) {
    clearInterval(deleteTimer);
    const btn = document.getElementById(btnId);
    if(btn) { btn.disabled = true; btn.innerText = originalText; }
}
...
async function loadEnvConfig() {
    try {
        const res = await fetch('/api/config/env'); const data = await res.json();
        if (data.status === 'success') { currentEnvConfig = data.data; renderEnvConfig(); }
    } catch(e) { document.getElementById('env-list-container').innerHTML = "<div class='log-error'>解析 .env 配置文件失败！</div>"; }
}
...
function renderEnvConfig() {
    ...
    for (const [key, val] of Object.entries(currentEnvConfig)) {
        ...
        if (isBool) {
            ...
            inputHtml = `<label class="switch" style="margin-top:0;"><input type="checkbox" onchange="window.updateEnvVal('${key}', this.checked ? 'true' : 'false')" ${isChecked}><span class="slider round"></span></label>`;
        } else {
            const safeVal = val.replace(/'/g, "&#39;").replace(/"/g, "&quot;");
            inputHtml = `<input type="text" class="glass-input" value="${safeVal}" onchange="window.updateEnvVal('${key}', this.value)" placeholder="输入变量值...">`;
        }
...
window.deleteEnvVar = function(key) {
    ...
    startDeleteCountdown('btn-confirm-env', '确认删除');
}
window.closeEnvDeleteModal = function() {
    currentEnvKeyToDelete = null; document.getElementById('env-delete-modal').classList.remove('show'); stopDeleteCountdown('btn-confirm-env', '确认删除');
}
window.executeEnvDelete = function() {
    if (!currentEnvKeyToDelete) return; delete currentEnvConfig[currentEnvKeyToDelete]; window.closeEnvDeleteModal(); renderEnvConfig(); 
}

window.addNewEnvVar = function() {
    document.getElementById('new-env-key').value = ''; document.getElementById('new-env-val').value = ''; document.getElementById('add-env-modal').classList.add('show'); setTimeout(() => document.getElementById('new-env-key').focus(), 100);
}
...
window.confirmAddEnvVar = function() {
    ...
    if (!cleanKey) { alert("键名不合法！只能包含字母、数字和下划线。"); document.getElementById('new-env-key').focus(); return; }
    if (currentEnvConfig[cleanKey] !== undefined) { alert("⚠️ 该变量已存在于列表中！"); document.getElementById('new-env-key').focus(); return; }
    currentEnvConfig[cleanKey] = valInput; window.closeAddEnvModal(); renderEnvConfig();
}
window.saveEnvConfig = async function() {
    const btn = document.getElementById('btn-save-env'); const msg = document.getElementById('save-env-msg'); 
    ...
    try {
        ...
        if (data.status === 'success') { msg.className = "log-success"; msg.innerText = "✔️ " + data.msg; } else { msg.className = "log-error"; msg.innerText = "❌ " + data.msg; }
...
window.toggleAutoScroll = function() { 
    ...
    if (autoScroll) { 
        btn.innerHTML = '<span class="material-symbols-outlined" style="font-size: 18px; vertical-align: middle;">swap_vert</span> 自动滚动: 开启'; 
        btn.style.background = "rgba(var(--monet-rgb), 0.15)"; 
    } else { 
        btn.innerHTML = '<span class="material-symbols-outlined" style="font-size: 18px; vertical-align: middle;">sync_disabled</span> 自动滚动: 暂停'; 
        btn.style.background = "rgba(191,97,106,0.3)"; 
    } 
}
