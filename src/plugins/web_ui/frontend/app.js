...
let ws = null;
window.switchTab = function(pageId, element, title) {
...
window.checkSystem = async function() { 
    ...
    try { 
        ...
        box.innerHTML = `<span class="log-info">[SYSTEM]</span>\n${JSON.stringify(await res.json(), null, 2)}`; 
    } catch (e) { 
        box.innerHTML = `<span class="log-error">[ERROR]</span> ${e.message}`; 
    } 
}

window.checkAI = async function() { 
    const box = document.getElementById('status-box'); 
    box.innerText = 'PING 大模型...'; 
    try { 
        const res = await fetch('/api/ai/status', {method: 'POST'}); 
        box.innerHTML = `<span class="log-success">[AI CORE]</span>\n${JSON.stringify(await res.json(), null, 2)}`; 
    } catch (e) { 
        box.innerHTML = `<span class="log-error">[ERROR]</span> 未响应`; 
    } 
}
...
function startDeleteCountdown(btnId, originalText) {
    ...
    let count = 10; 
    btn.disabled = true;
    btn.innerText = `${originalText} (${count}s)`;
    ...
    for (const [key, val] of Object.entries(currentEnvConfig)) {
        ...
        if (isBool) {
            ...
            inputHtml = `
                <label class="switch" style="margin-top:0;">
                    <input type="checkbox" onchange="window.updateEnvVal('${key}', this.checked ? 'true' : 'false')" ${isChecked}>
        ...
        container.innerHTML += `
            <div class="env-row">
                ...
                <button class="env-del-btn" onclick="window.deleteEnvVar('${key}')" title="永久删除此变量">🗑️</button>
            </div>
        `;
    }
}
...
window.deleteEnvVar = function(key) {
    ...
    startDeleteCountdown('btn-confirm-env', '🗑️ 确认删除');
}
...
window.closeEnvDeleteModal = function() {
    ...
    stopDeleteCountdown('btn-confirm-env', '🗑️ 确认删除');
}
...
window.saveEnvConfig = async function() {
    ...
    try {
        ...
        msg.className = "log-error"; msg.innerText = "❌ 传输异常";
    }
    ...
    try {
        ...
        data.plugins.forEach(p => {
            ...
            const toggleBtnHtml = p.can_disable ? 
                `<button class="glass-btn" onclick="window.togglePlugin('${p.raw_name}', '${isActive ? 'disabled' : 'active'}')">${btnText}</button>` : 
                `<button class="glass-btn glass-btn-disabled" disabled>🔒 防护</button>`;
                
            const deleteBtnHtml = p.can_delete ? 
                `<button class="glass-btn glass-btn-danger" onclick="window.openDeleteModal('${p.raw_name}')">🗑️ 销毁</button>` : 
                ``; 
        ...
        list.innerHTML = "<div class='log-error' style='text-align:center; padding:20px;'>⚠️ 获取元数据失败</div>"; 
    }
}
...
window.togglePlugin = async function(rawName, targetStatus) {
    try { 
        ...
        document.getElementById('plugin-list').innerHTML = "<div class='log-info' style='text-align:center; padding:20px;'>🔄 重新编排中...</div>"; 
...
window.openDeleteModal = function(rawName) { 
    currentPluginToDelete = rawName; 
    document.getElementById('delete-plugin-name').innerText = rawName.replace(/^_/, ""); 
    document.getElementById('delete-modal').classList.add('show'); 
    startDeleteCountdown('btn-confirm-plugin', '🗑️ 确认销毁');
}

window.closeDeleteModal = function() { 
    currentPluginToDelete = null; 
    document.getElementById('delete-modal').classList.remove('show'); 
    stopDeleteCountdown('btn-confirm-plugin', '🗑️ 确认销毁');
}
...
window.executeDelete = async function() {
    ...
    document.getElementById('plugin-list').innerHTML = "<div class='log-error' style='text-align:center; padding:20px;'>🗑️ 执行物理销毁...</div>";
    try { 
        ...
        if (data.status === 'success') { 
            ...
            alert(data.msg); loadPlugins(); 
        }
    } catch(e) { 
        alert("网络失败"); loadPlugins(); 
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    dropZone.addEventListener("dragover", (e) => { 
        ...
        dropMsg.innerText = "释放立即部署！"; 
    });
    dropZone.addEventListener("dragleave", () => { 
        dropZone.classList.remove("dragover"); 
        dropMsg.innerText = "点击或拖拽 .zip 包热安装"; 
    });
    ...
    async function handleFileUpload(file) {
        ...
        try { 
            ...
            if (data.status === 'success') { 
                ...
                setTimeout(() => { dropMsg.innerText = "部署完成"; loadPlugins(); }, 2000); 
            } else { 
                dropMsg.className = "log-error"; dropMsg.innerText = data.msg; 
            }
        } catch(e) { 
            dropMsg.className = "log-error"; dropMsg.innerText = "❌ 传输中断"; 
        }
    }
});
